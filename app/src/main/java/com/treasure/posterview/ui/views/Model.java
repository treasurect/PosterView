...
public class Model {
    ...
    private int defaultWidth;//防止从本地mipmap or drawable 中加载的图片被studio处理过，导致图片宽高与坐标点的比例不符
    ...
    public Model(Bitmap cover, List<Layer> layers, int defaultWidth) {
        this.cover = cover;
    ...
    public void setDrawWidth(int drawWidth) {
        ...
        drawCover = BitmapUtils.scaleBitmap(cover, scale, scale);
        for (Layer layer : layers)
    ...
    public void draw(Canvas canvas) {
        for (Layer layer : layers)
            if (!layer.isTouched && !layer.isPreSelect()) {
                if (flag) {
                    layer.caculateDrawLayer(drawWidth * 1.0f / defaultWidth);
                }
                layer.draw(canvas);
    ...
    private void switchLayer(Layer layer, Layer layer1) {
    ...
    public void setFocusLayer(Layer focusLayer) {
